...
export default function AuthSignup({ onSwitch }) {
  // Optimized: Consolidated all values into a single state object
  ...
  const handleSubmit = (e) => {
    ...
    // Base Validations
    if (!sanitizedFirstName || !sanitizedLastName || !sanitizedEmail || !sanitizedPhone || !formData.password || !formData.confirmPassword) {
      return setError("Fill all required fields.");
    }
    if (!validateName(sanitizedFirstName)) return setError("Enter a valid first name.");
    if (!validateName(sanitizedLastName)) return setError("Enter a valid last name.");
    if (!validateEmail(sanitizedEmail)) return setError("Enter a valid email.");
    ...
    // Conditional Account Validations
    if (showAccountDetails) {
      const sanitizedBankName = formData.bankName.trim();
      const sanitizedHolderName = formData.holderName.trim();
      const sanitizedAccountNumber = formData.accountNumber.trim();
      const sanitizedIfsc = formData.ifscCode.trim().toUpperCase();
    ...
    <div className="w-full max-w-md rounded-3xl border border-slate-100 bg-white p-6 sm:p-8 shadow-[0_15px_50px_rgba(15,23,42,0.03)]">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input type="password" value={formData.password} onChange={(e) => handleInputChange("password", e.target.value)} placeholder="••••••••" className="w-full h-11 px-4 rounded-xl border border-slate-100 bg-slate-50 text-sm focus:outline-none focus:border-slate-300" />
        </div>

        {/* Confirm Password */}
        <div>
          <label className="block text-[11px] font-bold uppercase text-slate-400 mb-1">Confirm Password</label>
          <input type="password" value={formData.confirmPassword} onChange={(e) => handleInputChange("confirmPassword", e.target.value)} placeholder="••••••••" className="w-full h-11 px-4 rounded-xl border border-slate-100 bg-slate-50 text-sm focus:outline-none focus:border-slate-300" />
        </div>
        ...
        <button type="submit" className="w-full h-12 bg-slate-950 hover:bg-slate-900 text-white rounded-full font-semibold transition-colors mt-2 col-span-1 sm:col-span-2">Create Account</button>
      </form>
