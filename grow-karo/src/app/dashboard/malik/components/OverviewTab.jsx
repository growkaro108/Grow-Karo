...
export default function OverviewTab({
  ...
  const pending = Array.isArray(withdrawals)
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={`${currency(pending.reduce((s, w) => s + w.amount, 0))}`}
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 xl:col-span-2">
          ...
          <div className="scrollbar-thin max-h-64 space-y-3 overflow-y-auto pr-1">
            {feed?.map((event) => (
              <div
                key={event.id}
                className="animate-feed-in flex items-start gap-3 border-b border-slate-800/60 pb-3 last:border-0"
              >
                <span
