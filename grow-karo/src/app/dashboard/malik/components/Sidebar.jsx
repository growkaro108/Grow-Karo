...
export default function Sidebar({
  ...
  const content = (
    <div className="flex h-screen flex-col">
      <div className="flex items-center gap-2.5 px-5 py-6">
        ...
        <div>
          <p className="font-display text-base font-semibold text-slate-100 leading-none">
            Vantage
          </p>
          ...
          return (
            ...
            >
              <span className="flex items-center gap-3">
                <item.icon
                  className={`h-4 w-4 ${isActive ? "text-emerald-400" : "text-slate-500 group-hover:text-slate-300"}`}
                />
                {item.label}
              </span>
              {!!count && (
                <span
                  className={`rounded-full px-2 py-0.5 text-[11px] font-mono ${isActive ? "bg-emerald-500/20 text-emerald-300" : "bg-slate-800 text-slate-400"}`}
                >
                  {count}
                </span>
              )}
  ...
  return (
    <>
      <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:border-r lg:border-slate-800 lg:bg-slate-900/40">
        {content}
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="animate-drawer-slide relative flex h-full w-72 flex-col bg-slate-900 shadow-2xl">
            <button
              onClick={onCloseMobile}
              className="absolute right-4 top-5 text-slate-400 hover:text-slate-200"
            >
              <X className="h-5 w-5" />
            </button>
            {content}
