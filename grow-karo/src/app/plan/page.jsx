...
const PlanDetailsPage = dynamic(() => import("./components/PlanDetailsPage"), {
  loading: TabLoader,
  ssr: false,
});
const EMPTY_ARRAY = [];
...
export default function Plans({ initialPlans = EMPTY_ARRAY }) {
    ...
    async (plan) => {
      if (!authUser?.id) {
        const response = await confirmMessage(
          "Please login to enroll in a plan",
          "Want to Login",
        );
        if (response) {
          router.push("/auth");
  ...
  const confirmEnrollment = useCallback(
    async (amount) => {
      ...
      setEnrolling(true);
      try {
        const response = await enrollInPlan(
        ...
        if (isSuccess) {
          setEnrolledSchemeIds((prev) => {
            ...
            if (prev && typeof prev === "object") {
              const existingArray = Array.isArray(prev.data)
                ? prev.data
                : Array.isArray(prev.schemeIds)
                  ? prev.schemeIds
                  : [];
